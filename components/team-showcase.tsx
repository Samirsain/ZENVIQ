      ...
      <div className="container relative z-10 mx-auto px-4 sm:px-6">
        ...
        >
          ...
          <p className="text-base sm:text-lg text-slate-500 max-w-xl mx-auto leading-relaxed">
            A tight-knit team of builders, designers & engineers who turn ideas
          ...
          {team.map((member, i) => (
            ...
            >
              ...
              <div className="relative bg-white rounded-3xl border border-slate-200/80 shadow-[0_1px_3px_rgba(0,0,0,0.04)] overflow-hidden hover:shadow-xl hover:border-slate-300/80 transition-all duration-500">
                {/* Gradient top bar */}
                <div
                  className={`h-1 w-full bg-gradient-to-r ${member.accent}`}
                />
                ...
                <div className="p-6 sm:p-8">
                  ...
                  <div className="relative mb-6">
                    <div
                      className={`w-24 h-24 rounded-2xl overflow-hidden border-2 ${member.accentBorder} shadow-lg group-hover:scale-105 transition-transform duration-500`}
                    >
                      <Image
                        src={member.image}
                    ...
                    {/* Role icon badge */}
                    <div
                      className={`absolute -bottom-2 -right-2 w-9 h-9 rounded-xl ${member.accentBg} ${member.accentBorder} border backdrop-blur-sm flex items-center justify-center shadow-sm group-hover:scale-110 transition-transform duration-300`}
                    >
                      <member.icon className={`w-4 h-4 ${member.accentText}`} />
                    </div>
                  </div>
                  ...
                  <div className="mb-5">
                    ...
                    <p
                      className={`text-sm font-semibold ${member.accentText} mb-3`}
                    >
                      {member.role}
                    </p>
                ...
                {/* Hover glow */}
                <div
                  className={`absolute inset-0 bg-gradient-to-br ${member.accent} opacity-0 group-hover:opacity-[0.03] transition-opacity duration-500 pointer-events-none rounded-3xl`}
                />
              </div>
            </motion.div>
          ))}
